...
import { LINE_COLORS, OUTER_R, INNER_R, LINE_OUTER_R, LINE_INNER_R, LINES } from '../constants';
...
const Wheel: React.FC<WheelProps> = ({
  ...
  // --- Line Segments (Outer Layer) - Hidden when expanded ---
  const lineSegments = useMemo(() => {
    if (expandedTask) return null; // Hide lines when sub-categories are expanded
  ...
  return (
    <div
      className="relative w-[600px] h-[600px] z-10 mx-auto no-select flex items-center justify-center"
    ...
    >
      ...
      {/* SVG Container */}
      <svg
        className={`absolute w-[600px] h-[600px] overflow-visible drop-shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all duration-300 ${isEditMode ? 'scale-95' : ''}`}
        viewBox="-300 -300 600 600"
        onClick={(e) => e.stopPropagation()}
      >
        <g onMouseLeave={() => { if (expandedTask) setExpandedTask(null); }}>{taskSegments}</g>
        <g className="origin-center">{lineSegments}</g>
        {subCategorySegments}
      </svg>

      {/* Close button when expanded */}
      {expandedTask && !isEditMode && (
        <button
          className="absolute top-2 right-2 p-2 bg-slate-800 rounded-full text-slate-400 hover:text-white hover:bg-slate-700 transition-colors z-30"
