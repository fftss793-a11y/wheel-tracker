...
function App() {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-between p-6 relative overflow-hidden bg-slate-950 transition-colors duration-500 ${isEditMode ? 'ring-8 ring-inset ring-blue-500/20' : ''}`}>
      ...
      {/* Background Decor */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full opacity-10 blur-[100px] pointer-events-none transition-colors duration-1000"
        style={{ backgroundColor: currentColor }}
      />
      {isEditMode && (
        <div className="absolute inset-0 bg-[linear-gradient(rgba(59,130,246,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.1)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none z-0"></div>
      )}
      ...
      <header className="w-full max-w-[1200px] flex justify-between items-start z-30">
        <div className="flex flex-col">
          <div className="flex items-center gap-2 text-slate-100">
            {isEditMode ? <PenTool className="w-6 h-6 text-blue-400" /> : <LayoutDashboard className="w-6 h-6 text-slate-400" />}
            <h1 className="text-xl font-bold tracking-tight">{isEditMode ? 'EDIT MODE' : 'PRODUCTION HUD'}</h1>
          </div>
          <p className="text-slate-500 text-xs font-mono mt-1 ml-8">
            {isEditMode ? 'CLICK ELEMENT TO CONFIGURE' : `SYSTEM READY // ${new Date(now).toLocaleTimeString()}`}
          </p>
        </div>

        <div className="flex gap-3 items-center">
          {/* Extra buttons shown only in edit mode */}
          {isEditMode && (
            <>
              <button onClick={() => setIsTemplateOpen(true)} className="p-3 bg-slate-900/50 backdrop-blur rounded-lg border border-slate-800 text-slate-400 hover:text-white hover:border-slate-600 transition-all" title="テンプレート">
                <Layers className="w-5 h-5" />
              </button>
              <button onClick={() => setIsHelpOpen(true)} className="p-3 bg-slate-900/50 backdrop-blur rounded-lg border border-slate-800 text-slate-400 hover:text-white hover:border-slate-600 transition-all" title="使い方">
                <HelpCircle className="w-5 h-5" />
              </button>
              <button onClick={() => setIsLogOpen(true)} className="p-3 bg-slate-900/50 backdrop-blur rounded-lg border border-slate-800 text-slate-400 hover:text-white hover:border-slate-600 transition-all" title="ログ">
                <FileText className="w-5 h-5" />
              </button>
              <button onClick={handleCsvExport} className="p-3 bg-slate-900/50 backdrop-blur rounded-lg border border-slate-800 text-slate-400 hover:text-white hover:border-slate-600 transition-all" title="CSV出力">
          ...
          {/* Current line indicator - shown next to settings when in edit mode */}
          {isEditMode && (
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-800/50 rounded-lg border border-slate-700">
              <span className="w-3 h-3 rounded-full" style={{ backgroundColor: LINE_COLORS[currentLine] }}></span>
              <span className="text-white text-sm font-bold">{currentLine}</span>
            </div>
          )}
          ...
          <button
            ...
            className={`p-3 backdrop-blur rounded-lg border transition-all ${isEditMode ? 'bg-blue-600 border-blue-500 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'bg-slate-900/50 border-slate-800 text-slate-400 hover:text-white hover:border-slate-600'}`}
          >
      ...
      {/* Main Wheel */}
      <main className="flex-1 flex flex-col items-center justify-center w-full relative z-20 scale-90 sm:scale-100 xl:scale-110 transition-transform pb-4">
        {isEditMode && <div className="absolute top-0 text-blue-400 text-sm font-bold animate-bounce uppercase tracking-widest">Click Ring to Edit Line</div>}
        <Wheel
      ...
      {/* Dashboard Panel */}
      <div
        className={`w-full max-w-[500px] z-30 mb-8 transition-all duration-300 ${isEditMode ? 'cursor-pointer hover:scale-105' : ''}`}
      ...
      >
        <div className={`bg-slate-900/80 backdrop-blur-md border rounded-2xl p-6 shadow-2xl relative overflow-hidden ${isEditMode ? 'border-blue-500/50 border-dashed' : 'border-slate-800'}`}>
          {isEditMode && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/40 text-blue-400 font-black text-xl tracking-widest z-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-slate-400 text-xs font-bold tracking-wider uppercase mb-1">Current Line</div>
              <div className="text-2xl font-black text-white flex items-center gap-3">
                <span className="px-2 py-0.5 rounded text-sm font-bold text-slate-950" style={{ backgroundColor: currentColor }}>{currentLine}</span>
                {config.lines[currentLine].name}
              </div>
            </div>

            <div className="text-right">
              <div className="text-slate-400 text-xs font-bold tracking-wider uppercase mb-1">Elapsed Time</div>
              <div className="text-3xl font-mono font-medium text-white tabular-nums tracking-tight flex items-center justify-end gap-2">
                {currentSession ? (
                  <><span className="animate-pulse text-red-500 text-xs">●</span>{formatDuration(now - currentSession.startedAt)}</>
          ...
          <div className="mt-6 pt-6 border-t border-slate-800 flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="text-slate-500 text-xs uppercase font-bold tracking-wider">Current Activity</div>
              <div className={`text-lg font-semibold truncate ${currentSession ? 'text-white' : 'text-slate-600'}`}>
                {currentSession ? currentSession.task : 'System Idle'}
              </div>
            </div>
            {currentSession && (
              <button
                onClick={() => setIsMemoOpen(true)}
                className="p-3 rounded-lg bg-slate-950 border border-slate-800 text-slate-400 hover:text-green-400 hover:border-green-500/50 transition-all"
                title="メモを追加"
              >
